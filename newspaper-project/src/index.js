...
class NewsArticlePreview extends React.Component {
  ...
  render() {
    return (
      <article onClick={this.sendID} className="article-preview">
        <div className="artTitle">{this.state.news.title}</div>
  ...
  async getNews(){
    const resp = await axios.get("https://newsapi.org/v2/top-headlines?country=ar&category=general&apiKey=750d667ad37c46b9b500c7479ad881bc");
    console.log(resp.data);
    this.setState({news: resp.data.articles})
  }
...
//   constructor(props) {
//     super(props);
//     this.state = { theme: ["sports","videogames"] };
